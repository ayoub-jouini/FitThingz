...
const EditExerciseModal: React.FC<Props> = (props) => {
  const {
  ...
  const [bodyPart, setBodyPart] = useState<string>(pBodyPart);
  const [target, setTarget] = useState<string>(pTarget);
  const [equipment, setEquipment] = useState<string>(pEquipment);
